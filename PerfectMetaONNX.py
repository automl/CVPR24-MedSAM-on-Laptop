...
def medsam_inference(prompt_encoder, mask_decoder, positional_encoding, img_embed, box_256, new_size, original_size):
    ...
    """
    sparse_embeddings, dense_embeddings = prompt_encoder.run(["sparse_embeddings", "dense_embeddings"], {"boxes":box_256[None, None, ...].astype(np.float32)})
    low_res_logits, iou = mask_decoder.run(["low_res_masks", "iou_predictions"], {"image_embeddings":img_embed, "image_pe": positional_encoding, "sparse_prompt_embeddings": sparse_embeddings, "dense_prompt_embeddings": dense_embeddings})
...
pos_encoding = np.load("onnxmodels/positional_encoding.npy")
pe = ort.InferenceSession("onnxmodels/lite_medsam_prompt_encoder.onnx")
sessions = dict()
def load_session(name):
    if name not in sessions:
        ie = ort.InferenceSession("onnxmodels/"+name+"_image_encoder.onnx")
        md = ort.InferenceSession("onnxmodels/"+name+"_mask_decoder.onnx")
...
def MedSAM_infer_npz_2D(img_npz_file):
    ...
    image_embedding = image_encoder.run(["image_embedding"], {"image":img_256})[0]
